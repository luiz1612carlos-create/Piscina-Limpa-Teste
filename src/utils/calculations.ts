...
/**
 * Calcula a distância de condução entre dois pontos com fallback agressivo e trava de segurança.
 */
export const calculateDrivingDistance = async (origin: Address | string, dest: Address | string): Promise<number> => {
    const fetchCoords = async (query: string): Promise<{ lat: string, lon: string } | null> => {
        // Limpeza: remove complementos após vírgula ou traço
        ...
        // Garante que a busca seja restrita ao Brasil e mais específica
        ...
        try {
            ...
            const response = await fetch(url, {
                headers: { 'User-Agent': 'PiscinaLimpa_SafeSearch_v4' }
            });
            ...
            if (data && data.length > 0) {
                // Validação básica: coordenadas próximas a 0,0 costumam ser erros de fallback do Nominatim
    ...
    const getResilientCoords = async (loc: Address | string): Promise<{ lat: string, lon: string }> => {
        ...
        // Tentativa 1: Endereço completo (Mais preciso)
        let coords = await fetchCoords(`${streetBase}, ${loc.number}, ${loc.city}, ${loc.state}, Brasil`);
        if (coords) return coords;

        // Tentativa 2: Rua e Cidade (Ignora número se estiver causando erro)
        coords = await fetchCoords(`${streetBase}, ${loc.city}, ${loc.state}, Brasil`);
        if (coords) return coords;

        // Tentativa 3: Cidade e Estado (Último recurso para evitar erro fatal)
    ...
    try {
        ...
        try {
            ...
            if (routeData.code === 'Ok' && routeData.routes?.length > 0) {
                ...
                // TRAVA DE SEGURANÇA: Se a distância for irreal para um serviço local (ex: > 500km), algo está errado
                return distKm > 500 ? 0 : distKm;
            }
        } catch (e) {
            console.warn("Rota real falhou, usando cálculo linear");
        }
        ...
        // Se a distância for absurda (> 500km), retornamos 0 para não cobrar KM indevido por erro de mapa
...
export const calculateClientMonthlyFee = (client: Partial<Client>, settings: Settings, overridePricing?: PricingSettings): number => {
    ...
    // CÁLCULO DE DISTÂNCIA COM TRAVA DE SEGURANÇA
    if (client.distanceFromHq && client.distanceFromHq > 0 && pricing.perKm) {
        // Se a distância for maior que 200km, consideramos erro de geocodificação e ignoramos a taxa de KM
        // para não gerar orçamentos de R$ 7.000,00 por engano.
