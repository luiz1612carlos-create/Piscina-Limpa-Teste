...
export const useAppData = (user: any | null, userData: UserData | null): AppData => {
    ...
    useEffect(() => {
        const unsub = db.collection('settings').doc('main').onSnapshot((doc: any) => {
            ...
            // Silenciamos o erro de permissão nas configurações para evitar poluição no console
            // se o usuário não estiver logado e as regras forem restritivas.
            if (err.code !== 'permission-denied') {
                console.warn("Aviso: Não foi possível carregar configurações.", err);
            }
            setSettings(defaultSettings);
            setLoadingState('settings', false);
        });
    ...
    useEffect(() => {
        const check = async () => {
            try {
                // Tenta buscar um admin. Se falhar por permissão, assumimos que o sistema já está configurado
                // ou que as regras de segurança estão ativas e bloqueando leituras anônimas.
                const s = await db.collection('users').where('role', '==', 'admin').limit(1).get();
                setSetupCheck(s.empty ? 'needed' : 'done');
            } catch (e) { 
                // Erro de permissão ou outro erro: assumimos 'done' para não mostrar o SetupView indevidamente
                setSetupCheck('done'); 
            }
        };
        check();
